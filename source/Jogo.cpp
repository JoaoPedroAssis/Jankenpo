    ...
    for (int lin = 0; lin < 6 ; lin++) {
        ...
        && Jogo::matriz_unidade[lin][11]->getVelocidade() > 0) {
            ...
            == PLAYER_MORREU) {
                ganhou = true;
                Mix_HaltMusic();
                Mix_PlayChannel(-1, Jogo::efeitos[GANHA], 0);
                printf("CPU Perdeu...");
            }
        ...
        && Jogo::matriz_unidade[lin][0]->getVelocidade() < 0) {
            ...
            == PLAYER_MORREU) {
                perdeu = true;
                Mix_HaltMusic();
                Mix_PlayChannel(-1, Jogo::efeitos[PERDE], 0);
...
bool Jogo::loadMidia() {
    ...
    // Load Chunk
    efeitos[PERDE] = Mix_LoadWAV("music/perdeu_jp.wav");
    if (efeitos[PERDE] == NULL) {
        printf("Failed to load Chunk explosao.wav!\n");
        success = false;
    }

    efeitos[GANHA] = Mix_LoadWAV("music/ondassonoras.wav");
    if (efeitos[GANHA] == NULL) {
        printf("Failed to load Chunk explosao.wav!\n");
        success = false;
    }
    ...
    for (int i = 0; i < 16; i++) {
        if (ultima_compra[i] == true) {
            if (i == 0) {
                ...
                // i%3 == 0 -> ação
                // do tipo tesoura e i%3 == 2 -> açao do tipo pedra
                return (i-1);
            } else {
                // ação do tipo papel
                return (i+2);
